...
*   terms in multiple imputation of incomplete three-level"                    *
*   Author:  Rushani Wijesuriya                                                *
*   16th of September 2020                                                     *
********************************************************************************

/*------------------------------------------------------------------------------
**NOTE: This script is for complete and missing data generation(section 4.1
and 4.2)
All MI approaches evaluated in the paper were implemented using R and Blimp
Please read the README file for instuctions on how to run the code
------------------------------------------------------------------------------*/
...
*STEP (ii):Simulate values for  child's age
gen c_age=runiform(7,10)

*STEP (iii):Simulate values for child's sex (female=0,male=1)
gen uran= runiform(0,1)
gen c_gender=0 if uran<=0.5 
replace c_gender=1 if uran>0.5
drop uran 

*STEP (iv):Simulate values of child SES
gen c_ses=rnormal(0,1)

*STEP (v):Simulate values for wave 1 NAPLAN scores
...
*STEP (vi):Expand the individuals to include 5 repeated observations (waves 2-6)
expand 6
...
*STEP (vii):Simulate values for depression (mean centered) scores at waves 2-6
gen e_c_dep=rnormal(0,1.5)
gen c_dep= -0.7+0.10*c_age +(-0.46)*c_gender+ (-0.01)*c_nap1_z+(-0.22)*c_ses + ///
            0.02*wave+ u_0i+ u_0ij+e_c_dep if wave==2 |wave==4|wave==6
drop e_c_dep

**Create a new variable for previous wave depression
bysort school child: gen prev_dep=c_dep[_n-1]
sort school child wave

*STEP (viii):Simulate NAPLAN scores at waves 3,5 and 7 using the substantive analysis model
...
*STEP (ix):Simulate values for the SDQ variable
...
/*------------------------------------------------------------------------------
Generating missing data (section 4.2) in depressive symptoms and SES values
------------------------------------------------------------------------------*/

**Generate post wave NAPLAN score
bysort school child: gen post_nap=napscore_z[_n+1]

**Generate missing data indicator for depressive symptoms according to a missing 
*at random  (MAR) mechanism in waves 2, 4 and 6 (0 missing, 1 observed)
...
**Generate missing values in SES at wave 1 according to a missing completely at 
...
*change working directory to a seperate folder for each simulation scenario to 
...
/*------------------------------------------------------------------------------
Analysis model 2: Interaction between the time-varying exposure and a time-fixed
                  baseline variable 
------------------------------------------------------------------------------*/
