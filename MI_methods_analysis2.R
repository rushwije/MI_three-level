...
#                   between the time-varying exposure and time-fixed confounder 
#                                                                             
...
# 3.SMC-JM-2L-DI  (50 sets of 1000/50 repetitions)
# 4.SMC-SM-2L-DI  (10 sets of 1000/10 repetitions)
...
for (i in 1:length(data)) {
    ...
    mods <- lapply(mylist, function(d) {
        lmer(napscore_z ~ prev_dep + time + prev_dep * c_ses + c_age + c_gender + 
...
for (i in 1:length(data)) {
    ...
    ## Generate new variables for the passive product terms
    simdataw <- data.frame(simdataw, Nap3.SES = NA, Nap5.SES = NA, Nap7.SES = NA, 
        Dep2.Nap3 = NA, Dep4.Nap5 = NA, Dep6.Nap7 = NA)
    ...
    pred["c_dep.2", c("school", "c_age", "c_gender", "c_ses", "c_nap1_z", "p_sdq.2", 
        "napscore_z.3", "p_sdq.4", "c_dep.4", "napscore_z.5", "p_sdq.6", "c_dep.6", 
        "napscore_z.7", "Nap3.SES")] <- codes
    
    pred["c_dep.4", c("school", "c_age", "c_gender", "c_ses", "c_nap1_z", "p_sdq.2", 
        "c_dep.2", "napscore_z.3", "p_sdq.4", "napscore_z.5", "p_sdq.6", "c_dep.6", 
        "napscore_z.7", "Nap5.SES")] <- codes
    
    pred["c_dep.6", c("school", "c_age", "c_gender", "c_ses", "c_nap1_z", "p_sdq.2", 
        "c_dep.2", "napscore_z.3", "p_sdq.4", "c_dep.4", "napscore_z.5", "p_sdq.6", 
        "napscore_z.7", "Nap7.SES")] <- codes
    
    pred["c_ses", c("school", "c_age", "c_gender", "c_nap1_z", "p_sdq.2", "c_dep.2", 
        "napscore_z.3", "p_sdq.4", "c_dep.4", "napscore_z.5", "p_sdq.6", "c_dep.6", 
        "napscore_z.7", "Dep2.Nap3", "Dep4.Nap5", "Dep6.Nap7")] <- c(codes, 1, 1)
    ...
    ## Derive interactions using passive imputation
    ...
    ## Visit sequence (defines the order in which the variables are to be imputed)
    visit <- c("c_ses", "Nap3.SES", "Nap5.SES", "Nap7.SES", "c_dep.2", "Dep2.Nap3", 
        "c_dep.4", "Dep4.Nap5", "c_dep.6", "Dep6.Nap7")
    
    ## Perform imputations
    imp2 <- mice(data = simdataw, m = M, predictorMatrix = pred, method = meth, visit = visit, 
        maxit = 10, allow.na = TRUE)
    ...
    FCSslwidepassivec_results.ICC[1, i] <- MI_est$var.comp[2, 1]/(MI_est$var.comp[2, 
        1] + MI_est$var.comp[1, 1] + MI_est$var.comp[3, 1])
    FCSslwidepassivec_results.ICC[2, i] <- (MI_est$var.comp[2, 1] + MI_est$var.comp[1, 
...
for (i in 1:length(temp)) {
    ...
    ## Generate new variables for the product terms
    simdataw <- data.frame(simdataw, Nap3.SES = NA, Nap5.SES = NA, Nap7.SES = NA, 
        Dep2.Nap3 = NA, Dep4.Nap5 = NA, Dep6.Nap7 = NA)
    ...
    pred["c_dep.2", c("school", "c_age", "c_gender", "c_ses", "c_nap1_z", "p_sdq.2", 
        "napscore_z.3", "p_sdq.4", "c_dep.4", "napscore_z.5", "p_sdq.6", "c_dep.6", 
        "napscore_z.7", "Nap3.SES", "Nap5.SES", "Nap7.SES")] <- codes
    ...
    pred["c_ses", c("school", "c_age", "c_gender", "c_nap1_z", "p_sdq.2", "c_dep.2", 
        "napscore_z.3", "p_sdq.4", "c_dep.4", "napscore_z.5", "p_sdq.6", "c_dep.6", 
        "napscore_z.7", "Dep2.Nap3", "Dep4.Nap5", "Dep6.Nap7")] <- codes
    ...
    FCSslwidepassiveall_results.ICC[1, i] <- MI_est$var.comp[2, 1]/(MI_est$var.comp[2, 
        1] + MI_est$var.comp[1, 1] + MI_est$var.comp[3, 1])
    FCSslwidepassiveall_results.ICC[2, i] <- (MI_est$var.comp[2, 1] + MI_est$var.comp[1, 
        1])/(MI_est$var.comp[2, 1] + MI_est$var.comp[1, 1] + MI_est$var.comp[3, 1])
}
...
for (i in 1:length(data)) {
    ...
    for (m in 1:M) {
        ...
        datL <- reshape(imputed, varying = list(c("prev_dep.3", "prev_dep.5", "prev_dep.7"), 
            c("napscore_z.3", "napscore_z.5", "napscore_z.7"), c("inter.3", "inter.5", 
                "inter.7")), idvar = "id", v.names = c("prev_dep", "napscore_z", 
            "inter"), times = c(3, 5, 7), direction = "long")
...
for (i in 1:length(data)) {
    ...
    imp4 <- mice(simdataw, m = M, maxit = 10, predictorMatrix = pred, method = meth, 
        seed = 2384, visit = visit, allow.na = TRUE)
    ...
    for (m in 1:M) {
        ...
        # 2. Remove unwanted variables
        imputed <- imputed[, names(imputed) %in% c("napscore_z.3", "napscore_z.5", 
            "napscore_z.7", "c_age", "c_gender", "c_dep.2", "c_dep.4", "c_dep.6", 
            "c_ses", "c_nap1_z", "c_id", "school")]
        
        # 3. Rename variables for reshape
        colnames(imputed)[colnames(imputed) == "c_dep.2"] <- "prev_dep.3"
        colnames(imputed)[colnames(imputed) == "c_dep.4"] <- "prev_dep.5"
        colnames(imputed)[colnames(imputed) == "c_dep.6"] <- "prev_dep.7"
    ...
    FCSmlwidepassivec_results.ICC[1, i] <- MI_est$var.comp[2, 1]/(MI_est$var.comp[2, 
        1] + MI_est$var.comp[1, 1] + MI_est$var.comp[3, 1])
    FCSmlwidepassivec_results.ICC[2, i] <- (MI_est$var.comp[2, 1] + MI_est$var.comp[1, 
        1])/(MI_est$var.comp[2, 1] + MI_est$var.comp[1, 1] + MI_est$var.comp[3, 1])
...
for (i in 1:length(data)) {
    ...
    ## visit sequence #updates the derived variables that depend on the target
    ## variable (defines the order in which the variables are to be imputed)
    visit <- c("c_ses", "Nap3.SES", "Nap5.SES", "Nap7.SES", "c_dep.2", "Dep2.Nap3", 
        "c_dep.4", "Dep4.Nap5", "c_dep.6", "Dep6.Nap7")
    ...
    FCSmlwidepassiveall_results.ICC[1, i] <- MI_est$var.comp[2, 1]/(MI_est$var.comp[2, 
        1] + MI_est$var.comp[1, 1] + MI_est$var.comp[3, 1])
    FCSmlwidepassiveall_results.ICC[2, i] <- (MI_est$var.comp[2, 1] + MI_est$var.comp[1, 
        1])/(MI_est$var.comp[2, 1] + MI_est$var.comp[1, 1] + MI_est$var.comp[3, 1])
...
for (i in 1:length(data)) {
    ...
    for (m in 1:M) {
        ...
        datw <- datw[, names(datw) %in% c("napscore_z.3", "napscore_z.5", "napscore_z.7", 
            "c_age", "c_gender", "c_dep.2", "c_dep.4", "c_dep.6", "c_dep.ses.2", 
            "c_dep.ses.4", "c_dep.ses.6", "c_ses", "c_nap1_z", "id", "simdataw$school")]
...
for (i in 1:length(data)) {
    ...
    # clusters)
    formula <- as.formula(napscore_z ~ prev_dep + time + prev_dep * c_ses + c_age + 
...
for (i in 1:length(data)) {
    ...
    ## Fit the analysis of interest on the imputed datasets
    mods <- lapply(mylist, function(d) {
        lmer(napscore_z ~ prev_dep + wave + prev_dep * c_ses + c_age + as.factor(c_gender) + 
            c_nap1_z + c_ses + (1 | school/c_id), data = d)
    })
